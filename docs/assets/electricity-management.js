(function start(){
  function init(){
    ...
    if(!C){ console.error('Chart.js not available'); return; }

    // ---- تنظیمات سراسری
    C.defaults.font.family = "'Vazirmatn', sans-serif";
    C.defaults.plugins.tooltip.rtl = true;
    C.defaults.plugins.legend.rtl = true;
    C.defaults.plugins.legend.labels.usePointStyle = true;

    // ---- نمودار 1
    const demandEl = document.getElementById('demandConsumptionChart');
    if (demandEl) new C(demandEl.getContext('2d'), {
      type: 'line',
      data: {
      ...
      options:{
        ...
        scales:{y:{beginAtZero:false,title:{display:true,text:'مگاوات (MW)'},ticks:{callback:v=>new Intl.NumberFormat('fa-IR').format(v)}}},
    ...
    // ---- نمودار 2
    const cityEl = document.getElementById('cityConsumptionChart');
    if (cityEl) new C(cityEl.getContext('2d'), {
      type:'bar',
      data:{
    ...
    // ---- نمودار 3
    const brkEl = document.getElementById('householdBreakdownChart');
    if (brkEl) new C(brkEl.getContext('2d'), {
      type:'doughnut',
