      ...
      .then(async function(r){
        ...
        waterKernel.onReady('cy', (C) => {
          ...
          try {
            ...
            const graph = gs;
            const elements = { nodes: graph.nodes, edges: graph.edges };
            console.debug('[CLD] will add', { nodes: elements.nodes.length, edges: elements.edges.length, sampleNode: elements.nodes[0] });

            C.startBatch();
            // atomic import via json replaces existing elements
            C.json({ elements });
            C.endBatch();
