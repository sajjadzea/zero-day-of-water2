...
async function joinWindWeightsOnAll(){
  ...
  const labelFa = p => (p?.['name:fa'] || p?.['alt_name:fa'] || p?.name || '—');
    ...
    async function ensureAdminBase(){
      ...
      countiesStroke = L.geoJSON(countiesGJ, {
        pane:'boundary', renderer:canvasRenderer,
        style:{ fill:false, color:'#111827', weight:1.5 }
      });
      ...
      console.log('[AHA] county source=amaayesh/counties.geojson');
      console.log('[AHA] boundary features=', countiesGJ?.features?.length);
...
async function actuallyLoadManifest(){
  ...
  try {
    ...
    console.log('[AHA] manifest path used=', url);
  ...
  function openTopModal(rows){
    ...
    // === WIND: load computed datasets (amaayesh/counties.geojson + amaayesh/wind_sites.geojson) ===
    {
      ...
      const radiusFromMW = mw => Math.max(5, 1.6*Math.sqrt(Math.max(0, mw||0)));
        ...
        const countiesFile = window.__AMA_COUNTIES_SOURCE;
        window.__AMA_countySource = 'counties.geojson (authoritative)';
        countiesGeo = polysFC; window.countiesGeo = countiesGeo; window.polysFC = polysFC;
        // === Build ALL-counties source once ===
        try {
          const allL6 = (polysFC.__combined?.features || []).filter(f => String(f?.properties?.admin_level || '') === '6');
          const prefer = (typeof countiesFile === 'object' && Array.isArray(countiesFile?.features) && countiesFile.features.length)
            ? countiesFile
            : (allL6.length ? { type: 'FeatureCollection', features: allL6 } : polysFC);
          window.__countiesGeoAll = prefer;
          console.log('[AHA] counties(all).features=', (prefer?.features || []).length);
        } catch (_) {
          window.__countiesGeoAll = polysFC;
        }
        if (polysFC?.features?.length && countiesLayer) {
          ...
          if (boundary) {
            boundary.__AMA_PROTECTED = false;
            if (boundaryGroup) boundaryGroup.removeLayer(boundary); else map.removeLayer(boundary);
          }
          const boundarySrc = window.__countiesGeoAll || countiesGeo || polysFC;
          boundary = L.geoJSON(boundarySrc, {
            pane: 'boundary',
            style: { color: '#111827', weight: 1.5, fill: false }
          });
          boundary.__AMA_PROTECTED = true;
          if (boundaryGroup) boundaryGroup.addLayer(boundary); else boundary.addTo(map);
          if (boundary.bringToFront) boundary.bringToFront();
          map.fitBounds(boundary.getBounds(), { padding:[12,12] });
          map.setMaxBounds(boundary.getBounds().pad(0.25));
          boundary.setStyle({ className: 'neon-edge' });
          console.log('[AHA] boundary src features=', (boundarySrc?.features||window.__countiesGeoAll?.features||[]).length);
          console.log('[AHA] overlays list=', Object.keys(overlays||{}));
