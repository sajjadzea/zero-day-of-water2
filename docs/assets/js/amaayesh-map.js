...
async function joinWindWeightsOnAll(){
  ...
  const labelFa = p => (p?.['name:fa'] || p?.['alt_name:fa'] || p?.name || '—');
    ...
    async function __getAllCountiesFC() {
      ...
      // 1) سعی در استفاده از counties.geojson
      let fc = null;
      try {
      ...
      // 2) اگر ناکامل بود، از combined فقط admin_level=6 را بگیر
      if (!fc) {
        try {
      ...
      // 3) آخرین fallback (برای نریختن برنامه)
      if (!fc) fc = window.polysFC || { type:'FeatureCollection', features:[] };

      window.__countiesGeoAll = fc;
      console.log('[AHA] all-counties.features =', fc.features.length);
      return fc;
    }

    async function __refreshBoundary(map){
      const src = await __getAllCountiesFC();
      if (window.boundary) {
        if (boundaryGroup) boundaryGroup.removeLayer(window.boundary);
        else if (map?.removeLayer) map.removeLayer(window.boundary);
      }
      ...
      if (boundaryGroup) boundaryGroup.addLayer(window.boundary); else window.boundary.addTo(map);
      if (window.boundary.bringToFront) window.boundary.bringToFront();
      boundary = window.boundary;
      console.log('[AHA] boundary src features =', src.features?.length || 0);
    }
    ...
    async function ensureAdminBase(){
      ...
      console.log('[AMA] base groups protected:', !!baseAdminGroup, !!countiesFill?.__AMA_PROTECTED, !!boundary?.__AMA_PROTECTED);
    }
...
async function actuallyLoadManifest(){
  ...
  function openTopModal(rows){
    ...
    if(tabs.length){
      ...
      console.log('[AHA] baseData:', windPath, solarPath, damsPath);
