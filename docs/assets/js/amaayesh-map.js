...
/* === AMA PATCH: robust JSON loader + base resolver (do not duplicate) === */
(function(){
  if (window.__AMA_PATCH_APPLIED__) return;
  window.__AMA_PATCH_APPLIED__ = true;

  // Base detection that works on Netlify publish=docs
  const AMA_BASE = (()=>{
    if (location.pathname.includes('/amaayesh/')) return '/amaayesh/';
    return 'amaayesh/';
  })();

  // Safe JSON fetch: جلوگیری از HTML/SPA fallback و ایجاد چندین تلاش با پایه‌های مختلف
  async function fetchJSON(relOrAbs){
    const candidates = [];
    if (/^https?:\/\//.test(relOrAbs) || relOrAbs.startsWith('/')) candidates.push(relOrAbs);
    candidates.push(AMA_BASE + relOrAbs.replace(/^\/+/,''));
    candidates.push('/' + relOrAbs.replace(/^\/+/,''));
    let lastErr;
    for (const url of candidates){
      try{
        ...
        if (text.trim().startsWith('<!DOCTYPE')) throw new Error(`HTML received ${url}`);
        return JSON.parse(text);
      }catch(e){ lastErr = e; }
    }
    throw lastErr || new Error('fetchJSON failed for ' + relOrAbs);
  }

  // مانيفست سبک
  async function loadManifest(){
    const m = await fetchJSON('layers.config.json').catch(()=> ({}));
    window.__AMA_MANIFEST = m;
    console.info('[AHA] manifest files =', m?.files || []);
    return m;
  }

  // اکسپورت نرم برای ماژول بعدی
  window.__AMA_UTILS__ = { AMA_BASE, fetchJSON, loadManifest };
})();
...
async function ensureAdminBase(map){
  ensurePanes(map);
  const url = window.__AMA_MANIFEST?.baseData?.counties || 'counties.geojson';
  const gj = await window.__AMA_UTILS__.fetchJSON(url);
...
const AMA_OVERLAYS = {};
window.overlays = AMA_OVERLAYS;
async function loadPoints(key, url, toRadius){
  const data = await window.__AMA_UTILS__.fetchJSON(url);
...
async function ama_bootstrap(){
  ...
  await window.__AMA_UTILS__.loadManifest();
  await ensureAdminBase(window.map);

  const base = window.__AMA_MANIFEST?.baseData || {};
  await Promise.all([
    loadPoints('wind',  base.wind  || 'wind_sites.geojson',  f=>Math.max(4, Math.min(12, +f?.properties?.capacity_mw/20 || 6))),
    loadPoints('solar', base.solar || 'solar_sites.geojson', f=>8),
    loadPoints('dams',  base.dams  || 'dams.geojson',       f=>7),
  ]);
...
(function(){
  ...
  on(search, 'keydown', e=>{
    ...
    if(!f) return;
    try{ const g=L.geoJSON(f); window.map.fitBounds(g.getBounds(),{padding:[24,24]}); g.remove(); }catch(_){/*noop*/}
  });

  on($('#btn-zoom-in'),  'click', ()=> window.map.zoomIn());
  on($('#btn-zoom-out'), 'click', ()=> window.map.zoomOut());
  on($('#btn-geolocate'),'click', ()=>{ try { window.map.locate({setView:true, maxZoom:10}); } catch(e){ if(window.AMA_DEBUG) console.log('geolocate blocked',e); } });
