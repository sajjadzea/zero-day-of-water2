...
function absFromManifest(rel){
  const base = window.__LAYER_MANIFEST_BASE || '/data/';
  const qs = (typeof window.__AMA_BUILD_ID !== 'undefined') ? `?v=${window.__AMA_BUILD_ID}` : '';
  const abs = new URL(rel.replace(/^\.?\//,''), location.origin + base).pathname;
  return abs + qs;
}
...
(async function(){
  ...
  try {
    ...
    const MANIFEST_BASE = new URL('.', url).pathname;
    window.__LAYER_MANIFEST_BASE = MANIFEST_BASE;
    if (AMA_DEBUG) console.info('[ama:manifest] base', MANIFEST_BASE);
  } catch (e) {
  ...
  // لایه‌ها در پن‌های جدا برای کنترل z-index
    ...
    (async () => {
      ...
      let combined = null;
      try {
        combined = await fetchJSONFromManifest('khorasan_razavi_combined.geojson');
      } catch (e) {
        if (window.showToast) showToast('عدم دسترسی به داده‌ها: khorasan_razavi_combined.geojson');
        return;
      }
    ...
    // === WIND: load computed datasets (counties.geojson + wind_sites.geojson) ===
    {
      ...
      const radiusFromMW = mw => Math.max(5, 1.6*Math.sqrt(Math.max(0, mw||0)));
        ...
        let polysFC = null;
        try {
          polysFC = await fetchJSONFromManifest('counties.geojson');
        } catch (e) {
          try { polysFC = await fetchJSONFromManifest('khorasan_razavi_combined.geojson'); } catch(_) {}
        }
        ...
        try {
          windSitesFC = await fetchJSONFromManifest('wind_sites.geojson');
        } catch (e) {
          if (window.showToast) showToast('عدم دسترسی به داده‌ها: wind_sites.geojson');
        }
