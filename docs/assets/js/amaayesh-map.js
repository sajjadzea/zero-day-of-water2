...
window.runWindSelfCheck = function(){
  console.log('[AMA] self-check started');
  try{
    ...
    const addRow = (p, src)=>{
      const name = (p?.county || p?.name_fa || p?.name || p?.NAME_2 || p?.NAME_1 || p?.shahrestan || p?.['نام'] || p?.['نام_شهرستان'] || '');
      const k = keyOf(name);
    ...
    if (window.__AMA_windIdx && typeof window.__AMA_windIdx === 'object'){
      Object.keys(window.__AMA_windIdx).forEach(k => namesIdx.add(keyOf(k)));
    }
    ...
    const summary = {
      mapCount: rows.length, hasData:has, noData:nod,
      onlyInMap, onlyInIdx,
  ...
  console.log('[AMA] self-check done');
  ...
  // لایه‌ها در پن‌های جدا برای کنترل z-index
    ...
    // === WIND: load computed datasets (amaayesh/counties.geojson + amaayesh/wind_sites.geojson) ===
    {
      ...
      const radiusFromMW = mw => Math.max(5, 1.6*Math.sqrt(Math.max(0, mw||0)));
        ...
        // helper: compute bbox from wind sites (minLon,minLat,maxLon,maxLat)
        async function __ama_bboxFromWindSites(){
          try{
            const windSitesFC = await fetchJSONFromManifest('amaayesh/wind_sites.geojson');
            let minLon=Infinity,minLat=Infinity,maxLon=-Infinity,maxLat=-Infinity;
            (windSitesFC?.features||[]).forEach(f=>{
              const g=f.geometry; if(!g) return;
              // handle Point or first coord of any type
              if(g.type==='Point' && Array.isArray(g.coordinates)){
                const [x,y]=g.coordinates; if(isFinite(x)&&isFinite(y)){ minLon=Math.min(minLon,x); minLat=Math.min(minLat,y); maxLon=Math.max(maxLon,x); maxLat=Math.max(maxLat,y); }
              }else{
                const coords=g.coordinates; const flat=(Array.isArray(coords)&&Array.isArray(coords[0]))?coords.flat(3):[];
                for(let i=0;i<flat.length;i+=2){
                  const x=flat[i], y=flat[i+1];
                  if(isFinite(x)&&isFinite(y)){ minLon=Math.min(minLon,x); minLat=Math.min(minLat,y); maxLon=Math.max(maxLon,x); maxLat=Math.max(maxLat,y); }
                }
              }
            });
            if(!isFinite(minLon)||!isFinite(minLat)||!isFinite(maxLon)||!isFinite(maxLat)) return null;
            // pad by 20% or a small absolute pad
            const padLon = (maxLon-minLon)*0.2 || 0.5;
            const padLat = (maxLat-minLat)*0.2 || 0.5;
            return [minLon-padLon, minLat-padLat, maxLon+padLon, maxLat+padLat];
          }catch(_){ return null; }
        }
        function __ama_centroid(feature){
          const g=feature?.geometry; if(!g) return [NaN,NaN];
          function avg(arr){ let x=0,y=0,n=0; arr.forEach(([X,Y])=>{ x+=X; y+=Y; n++; }); return n? [x/n,y/n]:[NaN,NaN]; }
          if(g.type==='Polygon'){ return avg(g.coordinates?.[0]||[]); }
          if(g.type==='MultiPolygon'){ return avg(g.coordinates?.[0]?.[0]||[]); }
          return [NaN,NaN];
        }
        function __ama_inBbox(lon,lat,b){ return Array.isArray(b) && lon>=b[0] && lon<=b[2] && lat>=b[1] && lat<=b[3]; }

        // if counties.geojson is too small, build counties from combined + bbox filter
        let useSmart = false;
        const cntCount = Array.isArray(polysFC?.features) ? polysFC.features.length : 0;
        if (!cntCount || cntCount < 15){
          useSmart = true;
          let combined=null;
          try { combined = await fetchJSONFromManifest('amaayesh/khorasan_razavi_combined.geojson'); } catch(_){ }
          const bbox = await __ama_bboxFromWindSites();
          if (combined && bbox){
            const filtered = (combined.features||[]).filter(f=>{
              const lvl = String(f?.properties?.admin_level||'');
              if (lvl !== '6') return false; // county-level
              const [cx,cy] = __ama_centroid(f);
              return isFinite(cx) && isFinite(cy) && __ama_inBbox(cx,cy,bbox);
            });
            if (filtered.length){
              polysFC = { type:'FeatureCollection', features: filtered };
              if (window.AMA_DEBUG) console.warn('[ama-data] using combined+bbox; counties=', filtered.length);
            }
          }
        }
        if (window.AMA_DEBUG){
          console.log('[ama-data] counties features =', Array.isArray(polysFC?.features) ? polysFC.features.length : 0, 'smart=', useSmart);
        }
        countiesGeo = polysFC;
